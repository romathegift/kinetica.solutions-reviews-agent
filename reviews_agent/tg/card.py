...
АВАРИЙНОЕ СООБЩЕНИЕ (R2)
------------------------
build_failure_text строит сообщение для случая, когда узел упал
окончательно. У него НЕТ клавиатуры и быть не может: одобрять нечего,
черновик не существует. Это уведомление, а не карточка решения.
"""
...
# Текст исключения может быть очень длинным (трейс провайдера, тело ответа).
# Оператору нужен опознавательный признак, а не полный трейс: он в логах.
MAX_ERROR_CHARS = 600
...
# Человекочитаемые имена узлов для аварийного сообщения. Оператор — персонал
# клиента, слово "retrieve" ему ничего не говорит.
NODE_LABELS = {
    "classify": "класифікація відгуку",
    "retrieve": "пошук фактів у базі знань",
    "generate": "генерація чернетки",
}
...
def build_failure_text(review_id: str, node: str, message: str, review_text: str = "") -> str:
    """
    Сообщение об окончательном отказе узла (R2).

    Отправляется БЕЗ клавиатуры: одобрять нечего, черновика не существует.
    Смысл сообщения — «отзыв не потерялся, но им придётся заняться руками».

    Текст отзыва включается, если он известен: без него оператору пришлось
    бы идти в базу за содержимым, чтобы понять, срочное это или нет.
    На падении classify текст уже есть (ingest отработал), на падении
    retrieve/generate — тем более.

    Техническое сообщение показываем как есть, обрезанным: оператор его
    не поймёт, но перешлёт разработчику, и это дешевле, чем «сталася
    помилка» без опознавательных знаков.
    """
    stage = NODE_LABELS.get(node, _esc(node))

    lines = [
        f"❌ <b>Відгук {_esc(review_id)} не оброблено</b>",
        f"Збій на етапі: <b>{stage}</b>. Повторні спроби вичерпано.",
        "",
        "<b>Потрібна ручна відповідь.</b>",
    ]
    ...
    if review_text:
        ...
        lines.append("<b>Текст відгуку</b>")
        lines.append(
            f"<blockquote>{_esc(_clip(review_text, MAX_REVIEW_CHARS))}</blockquote>"
        )

    lines.append("")
    lines.append("<b>Технічні деталі</b>")
    lines.append(f"<code>{_esc(_clip(message, MAX_ERROR_CHARS))}</code>")
