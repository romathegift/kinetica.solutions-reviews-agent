...
закрывает его на выходе. Форма процесса выяснена на R3: один контейнер,
один воркер uvicorn. Пул по-прежнему не заводим — отзывы приходят
единицами в час, а не тысячами в секунду, и коннект на вызов дешевле
пула, который надо держать живым и переоткрывать после обрыва.
"""
...
# Операции над нитью
# ======================================================================
def thread_exists(review_id: str) -> bool:
    """
    Отвечает на один вопрос: START по этому review_id уже запускался?

    Нужен приёму отзывов (api/service.py). Повторный START по существующей
    нити не начинает прогон заново — он ДОПИСЫВАЕТ второй прогон к той же
    нити, потому что thread_id = review_id. Для CLI это лечится флагом
    --reset вручную, для HTTP-эндпоинта нужен отказ до вызова графа.

    Отдельная функция, а не get_pause в try/except: приёму безразлично,
    на паузе нить или уже доведена до конца — обработанный отзыв не
    обрабатывают повторно ни в том, ни в другом состоянии. Использовать
    ради этого читатель нагрузки паузы значило бы спрашивать не то,
    что нужно, и зависеть от его исключений.

    НЕ АТОМАРЕН. Между этой проверкой и graph.invoke() успевает пройти
    другой запрос. Защита от гонки — задача вызывающего; в сервисе это
    множество «в работе» под asyncio-локом.
    ...
    with open_graph() as (graph, _):
        ...
        # created_at пуст ровно тогда, когда чекпоинтов по нити нет —
        # тот же признак, по которому get_pause и resume_thread
        # отличают отсутствующую нить от существующей.
        return bool(snapshot.created_at)
